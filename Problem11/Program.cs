...
namespace Problem11
{
    internal class Program : IProgramm<int>
    {
        ...
        public int Execute()
        {
            ...
            var xlength = grid.Count;
            var ylength = grid[0].Count;
            var totalsum = 0;

            // left and right
            for(var x = 0; x < xlength; x++)
            {
                for(var y = 0; y <= ylength - totalNumbers; y++)
                {
                    var numbers = grid[x].Skip(y).Take(totalNumbers);
                    getProductOfNumbers(numbers, totalsum, out totalsum);
                }
            }
            ...
            {
                for(var x = 0; x <= xlength - totalNumbers; x++)
                {
                    var numbers = grid.Skip(x).Take(totalNumbers).Select(x => x[y]); ;
                    getProductOfNumbers(numbers, totalsum, out totalsum);
                }
            }
            ...
            for(var x = 0; x <= xlength - totalNumbers; x++)
            {
                for (var y = 0; y <= ylength - totalNumbers; y++)
                {
                    var numbers = new List<int> { grid[x][y], grid[x + 1][y + 1], grid[x + 2][y + 2], grid[x + 3][y + 3] };
                    getProductOfNumbers(numbers, totalsum, out totalsum);
                }
            }

            // diagonal right to left
            for (var x = xlength - 1; x >= totalNumbers - 1; x--)
            {
                for (var y = ylength - 1; y >= totalNumbers - 1; y--)
                {
                    var numbers = new List<int> { grid[x - 3][y], grid[x - 2][y - 1], grid[x - 1][y - 2], grid[x][y - 3] };
                    getProductOfNumbers(numbers, totalsum, out totalsum);
                }
            }
        ...
        private void getProductOfNumbers(IEnumerable<int> numbers, int totalsum, out int product)
        {
            var newproduct = numbers.Aggregate((a, b) => a * b);
            if (newproduct > totalsum)
            {
                product = newproduct;
                return;
            }
